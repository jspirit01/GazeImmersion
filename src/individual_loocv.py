...
def individual_model(data_path, fs, load_fs, print_test, load_pretrained_model, save_dir, log_path, seed):
    ...
    for i in user_list:
        ...
        for j in range(num_instance):
            ...
            valid_video = instance_list[num_instance-j-2]

            test_data = user_data[user_data["Video Code"].isin([test_video])]  
            valid_data = user_data[user_data["Video Code"].isin([valid_video])]
            train_data = user_data[~user_data["Video Code"].isin([valid_video, test_video])]
            out_put(f"valid video : {valid_video}, test video : {test_video}", log_path)

            x_train, y_train, x_valid, y_valid, x_test, y_test, columns = construct_dataset(train_data, valid_data, test_data, mode='individual')

            # Feature selection
            if fs:
                out_put(f"Shape of features used (before FS): {x_train.shape}, {x_valid.shape}, {x_test.shape}", log_path)
                ...
                out_put(f"Shape of features used (after FS): {x_train.shape}, {x_valid.shape}, {x_test.shape}", log_path)

            else:
                out_put(f"Shape of features used: {x_train.shape}, {x_valid.shape}, {x_test.shape}", log_path)
        ...
        valid_df.loc['mean'] = valid_df.mean(axis=0)
        ...
        total_valid_df.loc[i] = valid_df.loc['mean']
    ...
    total_valid_df.loc['acc'] = total_valid_df.mean(axis=0)
    ...
    out_put("\nVALID ACCURACY", log_path)
    out_put(total_valid_df, log_path)
    if print_test:
        out_put("\nTEST ACCURACY", log_path)
