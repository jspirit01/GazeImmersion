...
population_loocv.py
=========================
population model (person-independent model) with Leave-one-person-out cross-validation
...
def population_model(data_path, fs, load_fs, print_test, load_pretrained_model, save_dir, log_path, seed):
    ...
    for i in range(0, num_person):
        ...
        test_data = data[data["User Code"].isin([test_user])]  
        valid_data = data[data["User Code"].isin([valid_user])]
        train_data = data[~data["User Code"].isin([valid_user, test_user])]
        out_put(f"valid user : {valid_user}, test user : {test_user}", log_path)

        x_train, y_train, x_valid, y_valid, x_test, y_test, columns = construct_dataset(train_data, valid_data, test_data, mode='population')
        ...
        if fs:
            ...
            if load_fs:
                feature_list = np.load(f'pretrained_model/population_top10/feature/{i+1}fold_feature_list.npy')
            else:
